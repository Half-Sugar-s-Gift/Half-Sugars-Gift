# 半糖的禮物

## Select Language
> [簡體中文](README.md)   
> [繁體中文-當前文件](README_zh-hant.md)  
> [English](README_en.md)  
> [日本語](README_ja.md)  

| HSG 版本 | 支援 Among Us 版本 | 下載連結 |
|----------|-------------------|----------|
| 1.0.0    | 17.3.0            | [下載](https://github.com/hvtXsvc-skysilk/Half-sugar-s-gift/releases/download/v1.0.0/Half.sugar.s.gift.1.0.0.zip) |

### 加入我們的群組：
<a href="https://qun.qq.com/universal-share/share?ac=1&authKey=Xns8hwheNCb6PUuL%2BHKNid3VktgaQsMQ7QA%2Bt1mDAqjYGd0B%2FbrMDlqG4cZl3XEw&busi_data=eyJncm91cENvZGUiOiIzNjI0NzQ5NDUiLCJ0b2tlbiI6IlU5SUNsbXh4b2syQTJ5K1VIUXlTVWNCZVRWYXhMRjVWR0UxVkliRUF3UnRUS2NGaGxRUmQxTFhUWXgvVUMvWDgiLCJ1aW4iOiIxOTA4OTEzOTA0In0%3D&data=AjSzwAs1LMCESZnMxBTzBix6Yj-ZjcW-h_LaXyHlwd2TSoJpALBxKZkLSSuhuyI3UG8pFkqdo3Ozj0MXf739GfOIOt1D0_5xK6F_iuYieKU&svctype=5&tempid=h5_group_info">
  <img src="https://img.shields.io/badge/QQ-12B7F5?&logo=qq&logoColor=white&style=for-the-badge" alt="QQ" />
</a>

### 說明
- 本專案為 [**Nebula on the Ship**](https://github.com/Dolly1016/Nebula) 的職業插件，新增了一些職業以及指令。

> [!NOTE]
> 本專案的部分職業為 AI 創作。具體請查看 [來源文件](ATTRIBUTIONS.md)

### 匯入方法
1. 下載 [Nebula on the Ship](https://github.com/Dolly1016/Nebula) 模組並正確匯入，成功執行一次。
2. 在遊戲目錄下應新建的 **Addons** 目錄中加入本專案的壓縮包檔案。  
   *註：請勿解壓此壓縮包檔案*
3. 若無錯誤，則匯入完成。

> [!note]
> 若需下載懶人包，請前往 [半糖的小窩](https://github.com/hvtXsvc-skysilk/Half-sugar-s-gift/README.md/#半糖的小窩)

### 感謝以下貢獻者為本插件做出了貢獻
- 半糖 —— 策劃
- hvtXsvc —— 主開發者
- Nesplayer —— wiki 開發
- 曦曦 —— 主畫師

### 如何為本插件貢獻？
請查看 [貢獻文件](CONTRIBUTING.md)！
